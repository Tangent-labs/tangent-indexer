...
export class UserPointsRepository extends AbstractRepository {
  ...
  async computeUserPoints(startDate: Date, endDate: Date) {
    const sql = `
      ...
      -- Params: :start_at, :end_at as timestamptz (or change to timestamp consistently)
      WITH params AS (
        SELECT
          '${startDate.toISOString()}'::timestamptz AS start_at,
          '${endDate.toISOString()}'::timestamptz   AS end_at
      ),
      ...
      -- Compute raw points per segment
      seg_points AS (
        SELECT
          sp.user_task_id,
          sp.task_id,
          EXTRACT(EPOCH FROM (sp.seg_end - sp.seg_start)) / 3600.0 AS hours_in_window,
          sp.point_rate,
          sp.amount,
          sp.avg_price_usd,
          (sp.point_rate
          ...
          * COALESCE(sp.avg_price_usd, 0)
          ) AS points_raw
        FROM seg_price sp
      ),
      -- Aggregate per user_task
      per_task AS (
        SELECT
          user_task_id,
          task_id,
          ROUND(SUM(points_raw))::int AS points
        FROM seg_points
        GROUP BY user_task_id, task_id
      )
    ...
    await this.prismaClient.$queryRawUnsafe(sql)
  }
}
