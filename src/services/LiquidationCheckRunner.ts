...
import { LiquidationCheckService } from "./LiquidationCheckService.js"
...
export class LiquidationCheckRunner {
  private liquidationCheckService: LiquidationCheckService
  private liquidationContextService: LiquidationContextService
  ...
  constructor(
    liquidationCheckService: LiquidationCheckService,
    liquidationContextService: LiquidationContextService,
    ...
    this.liquidationCheckService = liquidationCheckService
    this.liquidationContextService = liquidationContextService
  ...
  async run() {
    ...
    try {
      ...
      const params = await this.liquidationCheckService.getLiquidationParams()
      ...
      if (this.context.isDbAlive) {
        this.liquidationCheckService.saveFiles({ markets, borrowers })
      }

      currentAction = "on_chain_data"
      const onChainData = await this.liquidationCheckService.getOnchainData(this.providers, markets, borrowers, this.marketViewerAddress as string)
      ...
      const { seizingList, liquidationList } = await this.liquidationCheckService.analyzeLiquidation(onChainData, borrowers)
      ...
      const prioritizedLiquidationList = this.liquidationCheckService.prioritizeActions(seizingList || [], liquidationList || [])
