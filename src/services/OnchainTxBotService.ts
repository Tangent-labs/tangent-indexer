...
export class OnchainTxBotService {
    ...
    async getOnChainData(marketsAddresses: string[], keeperAddresses: string[]) {
        ...
        try {
            onChainData = await chainView<[string[], string[]], OnchainTxBotStruct>(
                this.provider,
                OnchainTxBot.abi,
                OnchainTxBot.bytecode,
                [keeperAddresses, marketsAddresses]);
        }
        catch (e) {
    ...
    async updateIRAndRC(irsAndRcs: IRAndRC[], markets: any[], irCalc: string, rcCalc: string) {
        ...
        if (irToCheckpoint.length !== 0) {
            ...
            try {
                ...
                await this.telegramNotifierService.sendMessage(`IR of markets \`${irToCheckpoint.map(market => market.collatName).join(",")}\` have been checkpointed`)
            }
            catch (e: any) {
        ...
        if (rcToCheckpoint.length !== 0) {
            ...
            for (let i = 0; i < 5; i++) {
                try {
                    txCheckpointRC = await rewardAccumulator.processMultiRewards(rcToCheckpoint.map(market => market.marketAddress), await this.signer.getAddress(), i);
            ...
            if (isSuccess) {
                await this.telegramNotifierService.sendMessage(`RC of markets \`${rcToCheckpoint.map(market => market.collatName).join(",")}\` have been checkpointed`)
            }
            else {
