...
export class MarketActivityNotificationService {
  ...
  async sendNotifications(events: SortedEvents, marketNamesById: Map<number, string>): Promise<void> {
    for (const event of events.Deposit) {
      await this.sendEvent("Deposit", event, [`Staked amount: ${event.staked_amount}`], marketNamesById)
    }
    for (const event of events.ZapDeposit) {
      await this.sendEvent("ZapDeposit", event, [`Staked amount: ${event.staked_amount}`], marketNamesById)
    }
    for (const event of events.DepositAndBorrow) {
      await this.sendEvent("DepositAndBorrow", event, [`Staked amount: ${event.staked_amount}`, `Borrow amount: ${event.borrow_amount}`], marketNamesById)
    }
    for (const event of events.ZapDepositAndBorrow) {
      await this.sendEvent("ZapDepositAndBorrow", event, [`Staked amount: ${event.staked_amount}`, `Borrow amount: ${event.borrow_amount}`], marketNamesById)
    }
    for (const event of events.Withdraw) {
      await this.sendEvent("Withdraw", event, [`Withdrawn amount: ${event.withdrawn_amount}`], marketNamesById)
    }
    for (const event of events.Repay) {
      await this.sendEvent("Repay", event, [`Repaid amount: ${event.repaid_amount}`], marketNamesById)
    }
    for (const event of events.ZapRepay) {
      await this.sendEvent("ZapRepay", event, [`Repaid amount: ${event.repaid_amount}`], marketNamesById)
    }
    for (const event of events.RepayAndWithdraw) {
      await this.sendEvent("RepayAndWithdraw", event, [`Repaid amount: ${event.repaid_amount}`, `Withdrawn amount: ${event.withdrawn_amount}`], marketNamesById)
    }
    for (const event of events.ZapRepayAndWithdraw) {
      await this.sendEvent(
        "ZapRepayAndWithdraw",
        event,
        [`Repaid amount: ${event.repaid_amount}`, `Withdrawn amount: ${event.withdrawn_amount}`],
        marketNamesById
      )
    }
  }
